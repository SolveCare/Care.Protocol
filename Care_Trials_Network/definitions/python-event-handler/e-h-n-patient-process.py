...
class CustomPythonEventHandler(PythonEventHandler):
    ...
    def update_shared_trials(self, collection: str, questions_data: Map):
        ...
        for item in shared_data:
            ...
            item.put("Limitations", questions_data.get('Limitations'))
            item.put("Ethnicity", questions_data.get('Ethnicity'))
    ...
    def save_participant_answers(self, questions_data: Map):

        date = self.format_to_date_short(questions_data.get('createdAt'))
        print("participant answers date:", date)
        
        latitude = questions_data.get('UserLocationLatitude')
        longitude = questions_data.get('UserLocationLongitude')
        print("participant answers latitude:", latitude)
        print("participant answers longitude:", longitude)

        country = "None"

        parti_answers = {
            'ReferralCode': questions_data.get('ReferralCode'),
            'senderNodeAddress': questions_data.get('senderNodeAddress'),
            'Smoke': questions_data.get('Smoke'),
            'Gender': self.extract_gender(questions_data),
            ...
            'T&C': questions_data.get('T&C'),
            'userLocationLongitude': longitude,
            'Country': country,
            'Age': self.extract_age(questions_data),
            'Date': date
        }
        print("saving participant_answers", parti_answers)
        self.cdn.save('participant_answers', parti_answers)
        print("participant_answers saved", parti_answers)
    ...
    def handle(self) -> Map:
        ...
        print("participant answers-->")
        self.save_participant_answers(questions_data)
