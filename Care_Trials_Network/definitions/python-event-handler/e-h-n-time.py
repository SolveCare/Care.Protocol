...
class CustomPythonEventHandler(PythonEventHandler):
    ...
    def __date_string(self):
        ...
        print("[e-h-n-time.py] created_at: ", created_at)
    ...
    def __find_questions_data(self) -> List:
        ...
        print(f'[e-h-n-time.py] Found {len(questions_data_list)} records of 6_QUESTIONS')
    ...
    def __find_badges(self) -> List:
        ...
        print(f'[e-h-n-time.py] Found {len(badges_list)} records of BADGES')
        return badges_list

    def __update_badge_records(self, update_data):
        vault_filter = List.of(EqualitySearchFilter.builder().fieldName("senderNodeAddress").value(
            self.node.info().getScAddress()).build())
        print(f'[e-h-n-time.py] Update BADGES with data: {update_data}')
        updated = self.vault.update_by_query('BADGES', vault_filter, update_data)
        print(f'[e-h-n-time.py] Successfully updated {updated} BADGES')
    ...
    def __update_badge_record_if_required(self, badge):
        if self.__is_id_record() and self.__update_id_badge_required(badge.get("IDBadges")):
            ...
            print("[e-h-n-time.py 1] participant_activities_data: ", participant_activities_data)
            ...
            print("[e-h-n-time.py 2] participant_activities_data: ", participant_activities_data)
            self.cdn.save('participant_activities', participant_activities_data)
            ...
            print("[e-h-n-time.py 3] participant_activities_data: ", participant_activities_data)
            self.cdn.save('participant_activities', participant_activities_data)
            self.silver_records_badge_notification()

            print("[e-h-n-time.py helloo] participant_activities_data-->")
            ...
            print("[e-h-n-time.py 4] participant_activities_data: ", participant_activities_data)
            self.cdn.save('participant_activities', participant_activities_data)
            self.__update_badge_records(update_data)
    ...
    def __find_trials(self) -> List:
        ...
        print(f'[e-h-n-time.py] Found {len(trials_list)} records of TRIALS')
        return trials_list

    def __update_trials(self, update_data):
        vault_filter = List.of(EqualitySearchFilter.builder().fieldName("senderNodeAddress").value(
            self.node.info().getScAddress()).build())
        print(f'[e-h-n-time.py] Update TRIALS with data: {update_data}')
        updated = self.vault.update_by_query('TRIALS', vault_filter, update_data)
        print(f'[e-h-n-time.py] Successfully updated {updated} TRIALS')
    ...
    def __participants_details(self):
        ...
        if len(participants_data_list) > 0:
            ...
            print("[e-h-n-time.py 5] participants_details: ", participants_details)
            return participants_details
        else:
            print("[e-h-n-time.py] No question data found, so participants_details wil be empty")
            return " "
    ...
    def __save_records_data(self):
        ...
        data = {
            ...
            "medicalIdRecords": "Document are not specified",
        ...
        print("[e-h-n-time.py] Saving RECORD_DATA: ", data)
    ...
    def save_participant_data(self, action):
        ...
        print("[e-h-n-time.py 7] participant_activities_data: ", participant_activities_data)
        self.cdn.save('participant_activities', participant_activities_data)

    def handle(self) -> Map:
        print("[e-h-n-time.py] Start execution")
        ...
        print("[e-h-n-time.py] Execution done")
