...
class CustomPythonEventHandler(PythonEventHandler):
    ...
    def handle(self) -> Map:
        ...
        #sponsor_filter = List.of(EqualitySearchFilter.builder().fieldName("Country").value(self.event_payload.get("Country")).build())
        ...
        existing_sponsors = self.vault.search('PHYSICIAN_SPONSOR', sponsor_filter)
        throw_next_event = False
        current_date = self.current_milli_time()
        updatedMsg = "Purchased on : " + str(current_date)
        ...
        if len(existing_sponsors) > 0:
            ...
            existing_sponsor.put('physicianStatus', 'BOUGHT')
            existing_sponsor.put('updatedOn', updatedMsg)
            #existing_trial.put('status', 'LIKED')
            self.vault.update('PHYSICIAN_SPONSOR', sponsor_filter, existing_sponsor, False)
            #self.vault.update_all('TRIALS', sponsor_filter, Map.of("sponsorStatus", "LIKED"))
            
            
            self.context.getNotificationProvider().send(
            'Great news!',
            'You have successfully purchased the Physician.')
...
def execute(self: HandlerExecutionContext) -> Map:
    print(f'==> [eh-h-e-w-BUY-PIS.py]: {self}')
