...
class CustomPythonEventHandler(PythonEventHandler):
    ...
    def handle(self) -> Map:
        ...
        bought_filter = List.of(EqualitySearchFilter.builder().fieldName("senderNodeAddress").value(self.event_payload.get("senderNodeAddress")).build())
        ...
        data = {
            #'total_matched': self.readable(total_matched),
            #'total_notfit': self.readable(total_notfit),
            'total_noresponse': self.readable(total_noresponse),
        ...
        if participant_trials:
            ...
            updated = self.vault.update('PARTICIPANT_TRIALS', sender_filter, participant_trial, False)
            print(f'==> [updated trial]: {updated}')
        else:
