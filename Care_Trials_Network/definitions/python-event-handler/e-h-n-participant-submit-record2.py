...
class CustomPythonEventHandler(PythonEventHandler):
    ...
    def update_collection(self, collection_name, row, trial_filter):
        ...
        row.put("recordStatus","submitted")
        row.put("timeMessage", "Submitted on: "+current_date_string)
    ...
    def handle(self) -> Map:
        ...
        existing_data = self.vault.search('RECORD_DATA', vault_filter)
        print(f'==> [existing_data]: {existing_data}')
        for data in existing_data:
            data_status = data.get("recordStatus")
            #if data_status != "pending":
            data.put("recordStatus", "submitted")
