...
class CustomPythonEventHandler(PythonEventHandler):
    ...
    def handle(self) -> Map:
        ...
        print("self.node.info().getScAddress()",self.node.info().getScAddress())
        ...
        sponsor_filter = List.of(
            ...
            EqualitySearchFilter.builder().fieldName("specialization").value(self.event_payload.get("specialization")).build())
        sponsor_data_list = self.vault.search('SPONSOR_ANSWERS', sponsor_filter)
        ...
        self.vault.update('PHYSICIAN_SPONSOR', sponsor_filter, sponsor, True)
        self.context.getNotificationProvider().send(
            'Congratulations!',
            'You have a new Physician match. Check your matches.')
        return result
