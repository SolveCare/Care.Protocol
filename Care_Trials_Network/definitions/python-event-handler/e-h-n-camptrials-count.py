...
class CustomPythonEventHandler(PythonEventHandler):
    ...
    def handle(self) -> Map:
        print("Custom event handler -> [e-h-n-trials-count.py]")
        ...
        skipped_filter = List.of(skipped_criteria, sender_filter)
        skipped_data = self.vault.search('TRIALS_UNIQ', skipped_filter)
        skipped_count = len(skipped_data) 
        
        #bought_filter = List.of(EqualitySearchFilter.builder().fieldName("senderNodeAddress").value(self.sender_node_address).build())
        ...
        bought_filter = List.of(bought_criteria, sender_filter)
        ...
        noresponse_filter = List.of(noresponse_criteria, liked_criteria, sender_filter)
        ...
        print(f'total_matched {total_matched} for {self.sender_node_address}')
        print(f'total_notfit {total_notfit} for {self.sender_node_address}')
        print(f'total_noresponse {total_noresponse} for {self.sender_node_address}')
        ...
        print(f'total_noresponse data: {data}')
        
        vault_filter = List.of(EqualitySearchFilter.builder().fieldName("senderNodeAddress").value(self.sender_node_address).build())
        participant_trials = self.vault.search('PARTICIPANT_TRIALS', List.of(sender_filter))
        print(f'==> [participant_trials..no resp]: {participant_trials}')
        ...
        if participant_trials:
            ...
            updated = self.vault.update('PARTICIPANT_TRIALS',  List.of(sender_filter), data, False)
            print(f'==> [updated trial]: {updated}')
        else:
