...
class CustomPythonEventHandler(PythonEventHandler):
    ...
    def save_site(self, row: SearchResponse, questions_data: Map) -> bool:
        ...
        return self.save_trial(row, questions_data, False)

    def save_trial(self, row: SearchResponse, questions_data: Map, shared: bool) -> bool:
        row_data = row.getData()
        ...
        if row_data.get('CampaignType'):
            ...
            if campaign_max_distance < distance and not shared:
    ...
    def handle(self) -> Map:
        ...
        if len(questions_data_list) > 0:
            questions_data = questions_data_list[0]
            self.save_shared_trial(questions_data)
            self.re_match_trials(questions_data)
        else:
            self.save_shared_trial_no_questions()
        print('Execution done')
        return result

    def save_shared_trial(self, questions_data: Map):
        trial_id = self.event_payload.get('SiteID')
        trial = self.cdn.find_first('trials-campaign', SimpleQueryBuilder.eq('SiteID', trial_id))
        self.save_trial(trial, questions_data, True)

    def save_shared_trial_no_questions(self):
        self.save_shared_trial(HashMap())

    def re_match_trials(self, questions_data: Map):
